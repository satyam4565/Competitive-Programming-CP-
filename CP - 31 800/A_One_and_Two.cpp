...
void solve(){
    int n; cin >> n;
    vi a(n); frc(a, i, n);
    vi prefixCnt(n), suffixCnt(n);
    for(int i = 0; i < n; i++) {
        if(i == 0) prefixCnt[i] = (a[i] == 2);
        else prefixCnt[i] = prefixCnt[i-1] + (a[i] == 2);
    }
    for(int i = n-1; i >= 0; i--) {
        if(i == n-1) suffixCnt[i] = (a[i] == 2); 
        else suffixCnt[i] = suffixCnt[i+1] + (a[i] == 2);
    }
    for(int i = 0; i < n - 1; i++) {
        if(prefixCnt[i] == suffixCnt[i + 1]) {
            cout << i + 1 << "\n";
            return;
        }
    }
